...
contract SigReplay is ERC20 {
     ...
     */
    function badMind(address to, uint amount, bytes memory signature) public {
    ...
    // ECDSA验证
    function verify(bytes32 _msgHash, bytes32 memory _signature) public view returns (bool) {
        return ECDSA.recover(_msgHash, _signature) == signature;
    }
    // 记录已经mint的地址
    ...
    function goodMint(address to, uint amount,bytes memory signature) public {
        bytes32 _msgHash = toEthSignedMessageHash(getMessageHash(to, amount));
        require(verify(_msgHash, signature));
        // 检查该地址是否mint过
    ...
    function noinceMint(address to, uint amount, bytes memory signature) public {
        bytes32 _msgHash = toEthSignedMessageHash(keccak256(abi.encodePacked(
            to,
            amount,
            nonce,
            block.chainid
